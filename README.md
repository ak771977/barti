...
- Basket summaries are also appended to `state/baskets-<ENV>-<SYMBOL>.csv` (non-rotating, ENV is `testnet` or `live`) with open/close time, basket id, direction, levels, max volume, margin used, margin level, grid spacing, TP per lot, holding time (minutes), worst drawdown, and PnL.
